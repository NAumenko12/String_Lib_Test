...
static int parse_value(const char **str_ptr, const char *format_ptr, int width,
                       int length_modifier, va_list args, const char *str) {
  int value = 0;
  switch (*format_ptr) {
    case 'c':
...
static int parse_string(const char **str_ptr, int width, va_list args) {
  ...
  if (str) {
    ...
    while (**str_ptr && (!isspace(**str_ptr) || width != -1) &&
           (width == -1 || count < width)) {
      *str++ = **str_ptr;
      (*str_ptr)++;
      count++;
...
static int parse_octal(const char **str_ptr, int length_modifier,
                       va_list args) {
  unsigned long long int value = 0;
  const char *start_ptr = *str_ptr;
...
static void parse_length_modifier(const char **format_ptr,
                                  int *length_modifier) {
  switch (**format_ptr) {
    case 'h':
      *length_modifier = 1;
...
static int parse_input(const char *str, const char *format, va_list args) {
  ...
  while (*format_ptr) {
    if (*format_ptr != '%') {
      ...
      parse_format_specifier(&format_ptr, &flags, &width, &precision,
                             &length_modifier);
      result +=
          parse_value(&str_ptr, format_ptr, width, length_modifier, args, str);
      format_ptr++;
    }
  }
...
static void parse_format_specifier(const char **format_ptr, int *flags,
