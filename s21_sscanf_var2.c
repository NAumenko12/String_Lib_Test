...
// Функция для парсинга входной строки
static int parse_input(const char *str, const char *format, va_list args) {
    ...
    while (*format_ptr) {
        if (*format_ptr != '%') {
            // Пропуск пробелов в строке и формате
...
// Функция для парсинга спецификатора формата
...
// Функция для парсинга числа (ширины или точности)
...
// Функция для парсинга модификаторов длины
...
// Функция для парсинга значения в соответствии с форматом
...
// Функция для парсинга целого числа
static int parse_int(const char **str_ptr, int length_modifier, va_list args) {
    ...
    if (length_modifier == 1) {
        ...
        if (p) *p = (short int)(negative ? -value : value);
    } else if (length_modifier == 2) {
        long int *p = va_arg(args, long int *);
        if (p) *p = (long int)(negative ? -value : value);
    } else {
        int *p = va_arg(args, int *);
        if (p) *p = (int)(negative ? -value : value);
    }

    return (*str_ptr != start_ptr);
}

// Функция для парсинга числа с плавающей точкой
...
// Функция для парсинга восьмеричного числа
...
// Функция для парсинга беззнакового десятичного числа
...
// Функция для парсинга шестнадцатеричного числа
...
// Функция для парсинга адреса указателя
...
// Функция для парсинга количества прочитанных символов
